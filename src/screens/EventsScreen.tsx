  ...
  return (
    <Screen width="wide">
      ...
      <div className="flex items-center justify-between px-5 pt-2">
        <div>
          <h1 className="text-[28px] font-extrabold leading-tight text-text">{t('Події')}</h1>
          <p className="text-sm text-text-secondary">243 {t('подій поряд')}</p>
        </div>
      ...
      <div className="px-5 pt-4">
        <FilterChips<Cat>
          options={[
            { value: 'all', label: t('Усі'), icon: <Flag size={15} /> },
            { value: 'rally', label: t('Ралі'), icon: <Trophy size={15} /> },
            { value: 'track', label: t('Трек'), icon: <Flag size={15} /> },
            { value: 'tour', label: t('Тури'), icon: <Map size={15} /> },
          ]}
          value={cat}
          onChange={setCat}
        />
