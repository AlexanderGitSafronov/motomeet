  ...
  return (
    <Screen contentClassName="px-0">
      ...
      <div className="flex items-center gap-3 px-5 pt-2">
        ...
        <h1 className="text-2xl font-extrabold text-text">Налаштування</h1>
      </div>
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="block text-sm text-text-secondary">Перегляд і редагування профілю</span>
        </span>
        <ChevronRight size={20} className="text-text-muted" />
      </button>

      {/* Privacy & location */}
      <SectionLabel className="mt-6">Приватність і локація</SectionLabel>
      <div className="mt-3 flex flex-col gap-3 px-5">
        <SettingRow
          icon={<MapPin size={20} />}
          title="Приватність локації"
          subtitle="Лише друзі"
        ...
        <SettingRow
          icon={<EyeOff size={20} />}
          title="Режим-привид"
          subtitle="Приховати геолокацію"
          trailing={<Toggle checked={settings.ghostMode} onChange={() => toggleSetting('ghostMode')} label="Режим-привид" />}
        />
        <SettingRow
          icon={<Ruler size={20} />}
          title="Показувати відстань"
          subtitle="Показувати км іншим райдерам"
          trailing={<Toggle checked={settings.showDistance} onChange={() => toggleSetting('showDistance')} label="Показувати відстань" />}
        />
      </div>

      {/* Notifications */}
      <SectionLabel className="mt-6">Сповіщення</SectionLabel>
      <div className="mt-3 flex flex-col gap-3 px-5">
        <SettingRow
          icon={<Bell size={20} />}
          title="Push-сповіщення"
          subtitle="Звуки та банери"
          trailing={<ChevronRight size={18} className="text-text-muted" />}
        ...
        <SettingRow
          ...
          trailing={<Toggle checked={settings.eventReminders} onChange={() => toggleSetting('eventReminders')} label="Нагадування про події" />}
        />
        <SettingRow
          icon={<Radar size={20} />}
          title="Райдери поруч"
          subtitle="Сповіщати, коли друзі поруч"
          trailing={<Toggle checked={settings.nearbyRiders} onChange={() => toggleSetting('nearbyRiders')} label="Райдери поруч" />}
        />
      </div>

      {/* Appearance */}
      <SectionLabel className="mt-6">Вигляд</SectionLabel>
      <div className="mt-3 flex flex-col gap-3 px-5">
        <SettingRow
          icon={theme === 'dark' ? <Moon size={20} /> : <Sun size={20} />}
          title="Тема"
          subtitle={theme === 'dark' ? 'Темна тема' : 'Світла тема'}
          trailing={
            <div className="flex items-center rounded-full bg-surface-2 p-1">
              <button
                ...
                className={cn(
                  ...
                  theme === 'dark' ? 'bg-primary text-on-primary' : 'text-text-muted'
                )}
              >
                <Moon size={16} />
              </button>
              <button
                ...
                className={cn(
                  'grid h-8 w-9 place-items-center rounded-full transition-colors',
                  theme === 'light' ? 'bg-primary text-on-primary' : 'text-text-muted'
                )}
              >
                <Sun size={16} />
              </button>
